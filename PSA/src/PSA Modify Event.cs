...
namespace SmashAttacks
{
    public partial class FormModifyEvent : Form
    {
        ...
        private void DisplayParameter(int index)
        {
            ...
            if (paramType == 0)
            {
                if (lblEventId.Text == "06000D00") { if (lstParameters.SelectedItem.Equals("Flags"))
                    { paramType = 7; pnlFlagsBtn.Text = "Open Flag Properties"; } }
                if (lblEventId.Text == "06150F00") { if (lstParameters.SelectedItem.Equals("Special Flags"))
                    { paramType = 7; pnlFlagsBtn.Text = "Open Special Flag Properties"; } }
            }
        ...
        //  Display the parameter as a basic variable.
        public void ShowFlags(long value)
        {
            flagsValue.Text = p.Hex8(value);
        }
        ...
        private void pnlFlagsBtn_Click(object sender, EventArgs e)
        {
            if (pnlFlagsBtn.Text == "Open Flag Properties")
            {
                using (PSA_Flag_Properties ps = new PSA_Flag_Properties())
                {
                    ps.GetHexVal = flagsValue.Text;
                    if (ps.ShowDialog() == DialogResult.OK)
                    {
                        this.flagsValue.Text = ps.NewHexVal;
                    }
                }
            }
            else if (pnlFlagsBtn.Text == "Open Special Flag Properties")
            {

            }
        }

        private void flagsValue_Leave(object sender, EventArgs e)
        {
            if (lstParameters.SelectedIndex == -1) return;
            int index = lstParameters.SelectedIndex;

            try { eventData.parameters[index].word2 = p.UnHex(flagsValue.Text); }
            catch { eventData.parameters[index].word2 = 0; }
            flagsValue.Text = p.Hex8(eventData.parameters[index].word2);
        }
    }
}
